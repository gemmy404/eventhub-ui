  ...
  return (
    <article className="event-card">
      <div className="event-card__topline">
        <span className={`status-badge status-badge--${event.status.toLowerCase()}`}>{event.status}</span>
        {event.organizerName && <span className="event-card__organizer">By {event.organizerName}</span>}
      </div>
      <h2>{event.title}</h2>
      ...
      <Link className="event-card__link" to={`/events/${event.id}`}>View Event <span aria-hidden="true">→</span></Link>
    </article>
  )
}
